...
public final class FilesManager {
	 ...
	 */
	public static String estimateProjectRootFolderPath(MergeContextSt context){
	 ...
	 */
	public static String[] estimateFilesProjectFolderPath(MergeContextSt context){
		File left  = context.getLeft();
		File base  = context.getBase();
		File right = context.getRight();
	...
	public static String indentCode(String sourceCode){
		...
		try{
			...
			indentedCode = removeEmptyLines(indentedCode);
		} 
		return indentedCode;
	}

	/**
	 * Remove empty lines of given string.
	 * @param source
	 * @return
	 */
	private static String removeEmptyLines(String source) {
		source = source.replaceAll("(?m)^[ \t]*\r?\n", "");
		return source;
	}
	 ...
	 */
	public static boolean areFilesDifferent(File left, File base, File right,String outputFilePath, MergeContextSt context) {
		boolean result = true;
	...
	public static String prettyPrint(FSTNonTerminal node) {
		SimplePrintVisitorStructured visitor = new SimplePrintVisitorStructured();
		visitor.visit(node);
