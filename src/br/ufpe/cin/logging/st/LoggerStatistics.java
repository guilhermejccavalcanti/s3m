package br.ufpe.cin.logging.st;
...
import br.ufpe.cin.crypto.st.CryptoUtils;
import br.ufpe.cin.exceptions.st.CryptoException;
...
public class LoggerStatistics {
	...
	static{ 
		if(!STJFSTMerge.isCryptographed){
			try {
	...
	public static void logContext(String msg, MergeContextSt context) throws PrintException{
		try{
			initializeLogger();
	...
	private static void logFiles(String timeStamp, MergeContextSt context) throws IOException {
		try{
			//initialization
			String logpath = System.getProperty("user.home")+ File.separator + ".stjfstmerge" + File.separator;
			new File(logpath).mkdirs(); //ensuring that the directories exists	
