...
public class NewElementReferencingEditedOneHandlerTest {
	...
	public void testNereoMethodField() {
		...
		assertThat(mergeResult).contains("<<<<<<<MINEinta=15;|||||||inta=10;=======intm(){returna+15;}>>>>>>>YOURS");
	...
	public void testNereoFieldMethod() {
		...
		assertThat(mergeResult).contains("<<<<<<<MINEstaticintm(){return15;}|||||||intm(){return10;}=======inta=m();>>>>>>>YOURS");
		assertThat(ctx.newElementReferencingEditedOneConflicts).isOne();
	}
	...
	public void testNereoFieldField() {
		...
		assertThat(mergeResult).contains("<<<<<<<MINEinta=15;|||||||inta=10;=======intb=a+20;>>>>>>>YOURS");
		assertThat(ctx.newElementReferencingEditedOneConflicts).isOne();
	}
	...
	public void testNereoMethodMethod() {
		...
		assertThat(mergeResult).contains("<<<<<<<MINEstaticStringm(){return\"insidemethodmedited\";}|||||||Stringm(){return\"something...\";}=======voidn(){if(m().equals(\"something...\")){System.out.println(\"insidemethodn\");}}>>>>>>>YOURS");
		assertThat(ctx.newElementReferencingEditedOneConflicts).isOne();
	}
