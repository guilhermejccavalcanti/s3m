...
public class RealmProxyClassGenerator {
    ...
    // Class metadata for generating proxy classesprivate Elements elementUtils;
    ...
    public void generate() throws IOException, UnsupportedOperationException {
        ...
        className, // interfaces to implement
        "RealmObjectProxy").emitEmptyLine();
    ...
    private void emitAccessors(JavaWriter writer) throws IOException {
        for (VariableElement field : metadata.getFields()) {
            ...
            if (Constants.JAVA_TO_REALM_TYPES.containsKey(fieldTypeCanonicalName)) {
                ...
                writer.beginControlFlow("if (value == null)");
                // TODO: delete all the links instead
                writer.emitStatement("return");
                writer.endControlFlow();
    ...
    private void emitInitTableMethod(JavaWriter writer) throws IOException {
        ...
        "Table", // Method name
        "initTable", // Modifiers
        EnumSet.of(Modifier.PUBLIC, Modifier.STATIC), "ImplicitTransaction", // Argument type & argument name
        "transaction");
    ...
    private void emitCopyMethod(JavaWriter writer) throws IOException {
        ...
        className, // Method name
        "copy", // Modifiers
        EnumSet.of(Modifier.PUBLIC, Modifier.STATIC), "Realm", "realm", className, "newObject", "boolean", "update", "Map<RealmObject,RealmObjectProxy>", // Argument type & argument name
        "cache");
        if (metadata.hasPrimaryKey()) {
    ...
    private void emitUpdateMethod(JavaWriter writer) throws IOException {
        ...
        className, // Method name
        "update", // Modifiers
        EnumSet.of(Modifier.STATIC), "Realm", "realm", className, "realmObject", className, "newObject", "Map<RealmObject, RealmObjectProxy>", // Argument type & argument name
        "cache");
        for (VariableElement field : metadata.getFields()) {
            String fieldName = field.getSimpleName().toString();
            if (typeUtils.isAssignable(field.asType(), realmObject)) {
    ...
    private void emitEqualsMethod(JavaWriter writer) throws IOException {
        ...
        // FooRealmProxy aFoo = (FooRealmProxy)o
