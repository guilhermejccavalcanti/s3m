...
package io.realm.internal; import java.io.Closeable; import java.io.File; import java.io.IOException; import java.nio.ByteBuffer; /**
 ...
 */public class  Group implements Closeable {
	// Below values must match the values in realm::group::OpenMode in C++public static final int MODE_READONLY = 0;
	// Open in read-only mode. Fail if the file does not already exist.public static final int MODE_READWRITE = 1;
	// Open in read/write mode. Create the file if it doesn't exist.public static final int MODE_READWRITE_NOCREATE = 2;
	// Open in read/write mode. Fail if the file does not already exist.protected long nativePtr;
    ...
    // via the contextpublic void close() {
